...
export function BlacklistContent() {
  ...
  return (
    <>
      <Button onClick={() => setShow(prev => !prev)}>{show ? <>Скрыть</> : <>Показать</>}</Button>

      <ul className='space-y-5'>
        <li>
          <p>Высказывания на основе принадлежности к социальной группе</p>

          <WordsContainer show={show}>Пидор, пидорас, педик, гомосек</WordsContainer>
        </li>
        <li>
          <p>Высказывания по признакам пола, расы</p>

          <WordsContainer show={show}>Ниггер, пендос, жид</WordsContainer>
        </li>
        <li>
          <p>Высказывания, направленные на разжигание межнациональной розни</p>

          <WordsContainer show={show}>Хохол, кацап, хач</WordsContainer>
        </li>
        <li>
          <p>В негативном смысле, оскорбление кого-либо</p>

          <WordsContainer show={show}>Аутист, даун, симп, инцел, додик</WordsContainer>
        </li>
      </ul>
    </>
  )
...
function WordsContainer({ children, show }: PropsWithChildren<{ show: boolean }>) {
  ...
  return (
    <div className='relative my-2 flex select-none'>
      <small
        className={cn(
          'text-light-gray-color absolute left-0 opacity-0 transition-all duration-500',
          show ? 'translate-x-0 opacity-100' : '-translate-x-[10%]'
        )}
      >
        {wordsWithSuffix}
      </small>
