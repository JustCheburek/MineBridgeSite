...
export const generateMetadata = async ({ params }: ParamsProp): Promise<Metadata> => {
  ...
  const [Case, Drop] = await Promise.all([getCase({ name: CaseName }), getDrop({ name: DropName })])
...
export default async function DropsItems({ params }: ParamsProp) {
  const { Case: CaseName, Drop: DropName } = await params
  if (DropName !== 'all') {
  ...
  return (
    <div>
      ...
      <Section>
        {Drops.filter(Drop => Drop.name !== 'all').map((Drop, index) => (
          <Box key={Drop.name} casebox={index === 2}>
            <Text>
              <h3>{Drop.displayname}</h3>
              <Price>{Drop.price}</Price>
              <Url href={`/shop/drop/${CaseName}/${DropName}/${Drop.name}`} className='my-[10px]'>
                Выбрать
              </Url>
            </Text>
          </Box>
